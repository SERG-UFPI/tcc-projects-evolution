...
const CommitsLinesModal = (props: ModalProps) => {
      ...
      if (
        ...
        users.forEach((user) => {
          if (githubUsers.data['users'][user].indexOf(elem.author) != -1) {
            if (
              (elem.date == date && type == 'Adicionadas') ||
              (elem.date == date && type == 'Removidas')
            ) {
              result.push({
                id: count,
  ...
  const columns = [
    ...
    { label: 'Autor', accessor: 'author', sortable: true },
...
export default CommitsLinesModal;
